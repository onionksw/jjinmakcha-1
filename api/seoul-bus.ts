  ...
  try {
    ...
    if (routeNo) {
      try {
        ...
        // JSON이 아닌 응답(HTML/XML 오류)이면 응답 앞부분 노출
        if (routeText.trimStart().startsWith('<')) {
          _routeDebug = { authError: true, keyLen: ROUTE_KEY.length, keyPrefix: ROUTE_KEY.slice(0,4), responseHead: routeText.slice(0, 200) };
          throw new Error('busRouteInfo 비JSON 응답');
        }
        const routeData = JSON.parse(routeText);
        const routes = toItems(routeData);
        _routeDebug = { keyLen: ROUTE_KEY.length, keyPrefix: ROUTE_KEY.slice(0,4), routeCount: routes.length, routes };
